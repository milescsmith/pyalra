...
def test_choose_k(random_matrix, expected_k, expected_num_of_sds, expected_d):
    k, num_sds, d = choose_k(A_norm=random_matrix, K=10, noise_start=5, seed=1)

    assert k == expected_k
    assert_allclose(num_sds, expected_num_of_sds, atol=0.01)
    assert_allclose(d, expected_d, atol=0.001)
